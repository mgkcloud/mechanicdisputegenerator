    ...
    try {
        ...
        results.forEach((result, index) => {
            ...
            if (result.status === 'fulfilled') {
                ...
                // Process the fulfilled value to convert Markdown to HTML
                const sectionData = result.value; // e.g., { legalBasis: { summaryText: '**Markdown** text' } }
                const processedSectionData = {};

                for (const key in sectionData) {
                    if (Object.hasOwnProperty.call(sectionData, key)) {
                        const sectionContent = sectionData[key]; // e.g., { summaryText: '**Markdown** text' }
                        processedSectionData[key] = {};
                        
                        // Check nested keys like 'text' or 'summaryText' for Markdown
                        for (const contentKey in sectionContent) {
                            if (Object.hasOwnProperty.call(sectionContent, contentKey)) {
                                const value = sectionContent[contentKey];
                                // Convert if it's a string and the key suggests text content
                                if (typeof value === 'string' && (contentKey === 'text' || contentKey === 'summaryText' || contentKey === 'advice')) { // Add other keys if needed
                                    try {
                                        // Apply Markdown parser ONLY for paragraph breaks etc.
                                        // Assumes AI does NOT include markdown titles like **Title:**
                                        const htmlContent = marked.parse(value);
                                        // console.log(`[DEBUG] Original marked output for ${sectionName}.${key}.${contentKey}:`, htmlContent); // Keep if needed
                                        
                                        processedSectionData[key][contentKey] = htmlContent; // Use direct marked output
                                    } catch (parseError) {
                                        console.error(`Markdown parsing failed for ${sectionName}.${key}.${contentKey}:`, parseError);
                                        processedSectionData[key][contentKey] = value; // Fallback to original text on error
                                    }
                                } else {
                                    processedSectionData[key][contentKey] = value; // Keep non-string or other fields as is
                                }
                            }
                        }
                    }
                ...
                // Merge the *processed* data into generatedSections
                Object.assign(generatedSections, processedSectionData);
