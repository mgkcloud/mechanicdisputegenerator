...
export async function storeDocument(documentId, content, options = {}) {
  ...
  const isJsonFile = format === 'json' || contentType.includes('json');
  ...
  if (isJsonFile) {
    console.log(`\n⚠️ JSON FILE STORAGE - SPECIAL HANDLING ⚠️`);
    console.log(`Storage path: ${fileName}`);
    
    // Validate content is valid JSON to avoid issues
    if (typeof content === 'string') {
      try {
        // Parse and re-stringify to validate and normalize
        const parsed = JSON.parse(content);
        // Only re-stringify if parsing succeeded
        console.log(`JSON validation: Valid JSON content (${Object.keys(parsed).length} top-level keys)`);
      } catch (jsonError) {
        console.error(`⚠️ WARNING: Invalid JSON content being stored! Error: ${jsonError.message}`);
        console.error(`First 100 chars: ${content.substring(0, 100)}...`);
        // Continue with storage attempt anyway
      }
    } else {
      console.error(`⚠️ WARNING: Non-string content for JSON file: ${typeof content}`);
    }
  }
  console.log(`---------------------------\n`);
  ...
  try {
    ...
    if (R2_BUCKET) {
      ...
      await R2_BUCKET.put(fileName, content, {
        httpMetadata: {
          contentType,
...
export async function getDocument(documentKey) {
  try {
    ...
    if (R2_BUCKET) {
      ...
      // If it's a JSON file, validate that the content is actually valid JSON
      if (isJsonFile) {
        try {
          JSON.parse(content); // Just validate, don't actually use the result
          console.log(`Retrieved JSON is valid.`);
        } catch (jsonError) {
          console.error(`⚠️ WARNING: Retrieved JSON is invalid! Error: ${jsonError.message}`);
          console.error(`First 100 chars: ${content.substring(0, 100)}...`);
          // Return content anyway - calling code will handle JSON parsing
        }
      }
