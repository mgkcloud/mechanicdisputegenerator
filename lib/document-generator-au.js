...
export async function generateAustralianDocument(formData) {
  ...
  try {
    ...
    try {
      await storeDocument(filename, documentText, { 
      ...
      console.log(`Document stored successfully with filename: ${filename}`);
    } catch (storageError) {
      console.error(`Failed to store document ${filename}:`, storageError);
      // Decide how to handle storage failure - maybe return success but with a warning?
      // For now, re-throwing to indicate a critical failure.
      throw new Error(`Failed to store generated document: ${storageError.message}`);
    }
