  ...
  try {
    ...
    try {
      ...
      storedInputUrl = await storeDocument(filename, jsonString, { 
        ...
        format: "json", // Changed from "input.json" to "json" for correct extension
        metadata: { 
          originalDocumentFilename: filename,
          originalDocumentType: documentType,
          inputData: true // Add a marker to identify this as input data
        } 
      });
      
      // Attempt to verify the JSON was stored correctly - use correct filename format for verification
      const verificationCheck = await getDocument(`${filename}.json`);
      if (verificationCheck) {
        console.log(`✅ INPUT JSON VERIFIED: Successfully stored and retrieved ${filename}.json (${verificationCheck.length} bytes)`);
      } else {
        console.error(`❌ INPUT JSON VERIFICATION FAILED: Could not retrieve ${filename}.json after saving`);
      }
      
      console.log(`Original input data stored successfully as: ${filename}.json`);
      console.log(`------------------------------------------\n`);
    } catch (jsonError) {
